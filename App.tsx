...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#fdfdfd] text-slate-900 flex flex-col font-sans selection:bg-indigo-100 selection:text-indigo-900">
      {/* Editorial Navigation */}
      <nav className="sticky top-0 z-50 bg-white/70 backdrop-blur-xl border-b border-slate-200/40 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 h-24 flex items-center justify-between">
          <div className="flex items-center gap-4 group cursor-pointer" onClick={() => setActiveTab(Tab.MUSIC)}>
            <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-serif italic text-2xl shadow-xl transition-transform group-hover:scale-110">
              M
            </div>
            <div className="hidden sm:block">
              <h1 className="serif font-black text-2xl tracking-tight leading-none text-slate-900">MelodyMind</h1>
              <span className="text-[9px] font-black uppercase text-indigo-600 tracking-[0.4em] mt-1 block">A Daily Auditory Series</span>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-col items-end opacity-70">
             ...
             <span className="text-sm font-bold text-slate-900 tracking-tight">{new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
          </div>
        </div>
      </nav>
      ...
      <footer className="py-20 border-t border-slate-100 bg-white">
        <div className="max-w-5xl mx-auto px-6 text-center space-y-8">
          ...
          <div className="space-y-3">
            <p className="text-slate-400 text-[10px] font-black tracking-[0.4em] uppercase">The Melodymind Series</p>
            <p className="text-slate-500 text-sm max-w-xl mx-auto font-medium leading-relaxed">
              Curated by <span className="text-slate-900 font-bold">Gabriel Palansc</span> & synthesized by Gemini Intelligence. 
              Designed for the enduring pursuit of musical literacy.
            </p>
          </div>
        </div>
      </footer>
